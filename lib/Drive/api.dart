...
accessToken: googleAuth.accessToken,          idToken: googleAuth.idToken,
  ...
  Future<String?> _getFolderId(drive.DriveApi driveApi) async {
    ...
    String folderName = "اسم الحالةkk";
  ...
  Future<void> _uploadToNormal() async {
    try {
      ...
      var driveFile =  drive.File();
      final timestamp = DateFormat("yyyy-MM-dd-hhmmss").format(DateTime.now());
      driveFile.name = "test-$timestamp.txt";
      driveFile.modifiedTime = DateTime.now().toUtc();
      driveFile.parents = [folderId];

      // Upload
      final response =
      await driveApi.files.create(driveFile, uploadMedia:
      drive.Media(file.openRead(),file.lengthSync()));
